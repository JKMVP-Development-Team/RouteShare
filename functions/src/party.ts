...
export const createParty = onCall(async (request) => {
    ...
    if (!userId) {
        throw new HttpsError("unauthenticated", "User must be authenticated to create a party.");
    }
...
export const getPartyDetails = onCall(async (request) => {
    const userId = request.auth?.uid;   
    if (!userId) {
        throw new HttpsError("unauthenticated", "User must be authenticated to get party details.");    
    }
    ...
    if (!isMember) {
        throw new HttpsError("permission-denied", "User is not a member of this party.");
    }
...
export const getMembers = onCall(async (request) => {
    const userId = request.auth?.uid;
    if (!userId) {
        throw new HttpsError("unauthenticated", "User must be authenticated to get party members.");
    }
...
export const joinParty = onCall(async (request) => {
    const userId = request.auth?.uid;
    if (!userId) {
        throw new HttpsError("unauthenticated", "User must be authenticated to join a party.");
    }
    ...
    const partyData = partyDoc.data() as PartyDocument;
    const isAlreadyMember = partyData.members.some(member => member.userId === userId);
    if (isAlreadyMember) {
        throw new HttpsError("failed-precondition", "User is already a member of this party.");
    }
    
    if (partyData.isPrivate) {
        if (!inviteCode || inviteCode !== partyData.inviteCode) {
            throw new HttpsError("permission-denied", "Invalid invite code for private party.");
        }
    }
